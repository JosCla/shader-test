...
const static float brightThreshold = 0.9;
...
const static float movementMult = 0.5;
...
float4 HorizBlurPS(MainPixelInput input) : COLOR0
{
    ...
    for (int i = 1; i < NUM_SAMPLES; i++)
    {
        res += tex2D(
            ...
            input.TexCoord + float2(i * movement * (movementMult + biasX), 0)
        ) * gaussCoeffs[i];
        res += tex2D(
            SpriteTextureSampler,
            input.TexCoord - float2(i * movement * (movementMult - biasX), 0)
...
float4 VertBlurPS(MainPixelInput input) : COLOR0
{
    ...
    for (int i = 1; i < NUM_SAMPLES; i++)
    {
        res += tex2D(
            SpriteTextureSampler,
            input.TexCoord + float2(0, i * movement * (movementMult + biasY))
        ) * gaussCoeffs[i];
        res += tex2D(
            SpriteTextureSampler,
            input.TexCoord - float2(0, i * movement * (movementMult - biasY))
